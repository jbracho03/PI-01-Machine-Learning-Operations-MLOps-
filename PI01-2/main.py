...
datos = pd.read_csv(r"C:\Users\Roman Samuel Bracho\Desktop\PI01-2\Datos_PI.csv", low_memory = False)
...
def get_score_count(platform, score, year):
    if platform in ['n', 'd', 'h', 'a'] and int(score) == score and int(year) == year:         # Selecciona sólo las filas donde el id comience con la plataforma, score y ano sea el especificado
        ...
        return {"platform": platform_name, "ScoreMedia >": score, "year": year, "count": count}        # Devuelve un diccionario con los resultados
    else:
...
def get_actor(platform, year):  #Se asigna el nombre de la plataforma
